// HookeMinFinder.cpp: implementation of the HookeMinFinder class.
//
//////////////////////////////////////////////////////////////////////
...
#define      VARS		(10)	/* max # of variables	     */
...
// Following code is adapted from hooke.c, taken from NETLIB (www.netlib.org)
...
double HookeMinFinder::best_nearby(double * delta, double * point, double prevbest, int nvars)
{
	   double	   z[VARS];
	   double	   minf, ftmp;
	   int		   i;
...
int HookeMinFinder::hooke(int nvars, double * startpt, double * endpt, double rho,
...
double HookeMinFinder::f(double * x, int n)
{
	Parameters p(n);
	if (posonly) for (int j = 0 ; j < n ; j++) p.setValue(j,exp(x[j]));
	else for (int i = 0 ; i < n ; i++) p.setValue(i,x[i]);
	double y = function->evaluate(p);
	if (findMax) y = -y;
	return y;
}

/**
 * Finds location of the extreme of f.
 * The posonly can be used to transform the parameter space from strictly 
 * positive to -INF - +INF. Normally, a Function object should be used however
 * to achieve this transformation.
 *
 * WARNING: This function may 'travel' into negative range values for 
 * variables. 
 * 
 */
int HookeMinFinder::hooke(Function &f, Parameters &p, bool findMax, bool posonly)
{
	...
	for (int i = 0 ; i < nvars ; i++) startpt[i] = posonly ? log(p.getValue(i)) : p.getValue(i);
	int iters = hooke(nvars, startpt, endpt, rho, epsilon, itermax);
	for (int j = 0 ; j < nvars ; j++) if (posonly) p.setValue(j,exp(endpt[j])); else p.setValue(j,endpt[j]);
