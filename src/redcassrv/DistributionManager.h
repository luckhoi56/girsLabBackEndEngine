...
{
public:
	int add(GenericDistribution * dist);
	GenericDistribution * retrieve(int handle);
	GenericDistribution * remove(int handle);
...
private:
	...
	GenericDistribution * distributions[MAX_DISTRIBUTIONS];
	SyncMutex mutex;
};
